...
let highRatings = (rating) => {
    ...
    for (let j = 0; j < allMovies.length; j++) {
        if (allMovies[j].rating > rating) {
            console.log(allMovies[j].title + " has a rating of " + allMovies[j].rating);
            matches++;
        }
    }
...
let changeWatched = (title) => {
    for (let k = 0; k < allMovies.length; k++) {
        if (title == allMovies[k].title) {
            if (allMovies[k].haveWatched === true) {
                allMovies[k].haveWatched = false;
            } else {
                allMovies[k].haveWatched = true;
            }
        }
    }
